package com.winning.bi.view.component.common
{
	...
	[Event(name="enterKeyDown", type="flash.events.KeyboardEvent")]
	//--------------------------------------------------------------------------
	//
	// Begin Class
	//
	...
	public class AutoSuggest extends ComboBox
	{
		 ...
		 *
		 */
		public var willToChange:Boolean=true;
		...
		public function AutoSuggest()
		{
			...
			//By Chenqi
			BindingUtils.bindProperty(this, "prompt", this, "promptText");
			
			this.addEventListener(FocusEvent.FOCUS_IN, eSearchASFocusIn);
			this.addEventListener(FocusEvent.FOCUS_OUT, eSearchASFocusOut);
		}

		private var _searchFeild:String;

		[Inspectable(category="General", defaultValue="null")]
		public function get SearchFeild():String
		{
			return _searchFeild;
		}
		 ...
		 *  @private
		 */
		public function set SearchFeild(value:String):void
		{
			_searchFeild=value;
		}
...
//            }
		...
		/**
		 *whether to open when collection change
		 */
		private var _collectionChangeOpen:Boolean=true;

		public function get collectionChangeOpen():Boolean
		{
			return _collectionChangeOpen;
		}

		public function set collectionChangeOpen(value:Boolean):void
		{
			//By Chenqi
			if(!value)
			{
				this.removeEventListener(FocusEvent.FOCUS_IN, eSearchASFocusIn);
				this.removeEventListener(FocusEvent.FOCUS_OUT, eSearchASFocusOut);
				promptText="";
			}
			_collectionChangeOpen=value;
			invalidateProperties();
		}
		...
		override protected function keyDownHandler(event:KeyboardEvent):void
		{
			...
			if (!event.ctrlKey)
			{
				...
				{
					if (this.dataProvider != null && this.dataProvider.length > 0 && this.dropdown.visible == true && this.dropdown.selectedItem == null)
					{
						this.dropdown.selectedItem=this.dataProvider[0];
					}
					this.dispatchEvent(new KeyboardEvent("enterKeyDown"));
				}
			}
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			if (image)
			{
				image.x=unscaledWidth - image.measuredWidth - 5;
				image.y=unscaledHeight / 2 - image.measuredHeight / 2;
			}
		}
		...
		{
			var myLabel:String;
			if (SearchFeild == null || SearchFeild == "")
			{
				myLabel=itemToLabel(item);
				return true;
			}
			else
			{
				myLabel=item[SearchFeild];
			}
			...
			if (myLabel != null && myLabel.substr(0, text.length).toLowerCase() == prompt.toLowerCase())
			{
				return true;
			}
...
//                _autoCompleteSuggestChange=false;
		...
		//By Chenqi
		[Bindable]
		public var promptText:String=ResourceManager.getInstance().getString('localization','inputKeyWords');

		public var isStringFlag:Boolean=false;

		private function eSearchASFocusOut(event:FocusEvent):void
		{
			
			if (!isStringFlag && (selectedItem == null || StringUtil.trim(selectedItem.toString()) == ""))
			{
				prompt=promptText;
			}
			else if (!isStringFlag && selectedItem is String)
			{
				prompt=promptText;
			}
		}

		private function eSearchASFocusIn(event:FocusEvent):void
		{
