...
package com.winning.presentation.component.common.dgCombo
{
...
//--------------------------------------
	 ...
	 *
	 */
	public class DGComboBox extends ComboBase implements IDataRenderer, IDropInListItemRenderer, IListItemRenderer
	{
...
//            _dropdown.itemRenderer = value;
		...
		{
			if (DGComboBox.createAccessibilityImplementation != null)
				DGComboBox.createAccessibilityImplementation(this);
		}
...
//                _dropdown.labelFunction = _labelFunction;
			...
			if (selectionChanged)
			{
				if (!textChanged)
				{
					...
					else if (selectedIndex == -1 && prompt)
						textInput.text=prompt;
...
//            selectedIndexChanged = true;
		...
		public function open():void
		{
			...
			DataGrid(this.popUp).selectedItem=this.selectedItem;
		}
...
//            _dropdown.labelFunction = _labelFunction;
			...
			if(_dropdown.dataProvider && _dropdown.dataProvider.length < 5)
			{
				_dropdown.height=150;
				_dropdown.rowHeight=30;
			}
			...
			if (event is CollectionEvent)
			{
				...
				else if (ce.kind == CollectionEventKind.ADD)
				{
					if (collection.length == ce.items.length)
					{
						...
						if (selectedIndex == -1 && _prompt == null && !editable)
							selectedIndex=0;
					}
					else
					{
				...
				else if (ce.kind == CollectionEventKind.RESET)
				{
					...
					if (!selectedIndexChanged && !selectedItemChanged && !editable)
...
//            _showingDropdown = false;
		...
		override protected function keyDownHandler(event:KeyboardEvent):void
		{
			...
			else if (event.keyCode == Keyboard.ENTER)
			{
				if (_showingDropdown)
				{
					if (this.dataProvider != null && this.dataProvider.length > 0 && this.dropdown.selectedItem == null)
					{
